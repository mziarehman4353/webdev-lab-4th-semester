...
$product = $pdo->prepare("SELECT * FROM products WHERE id = ?")->execute([$id]) ? null : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $price > 0) {
        $stmt = $pdo->prepare("UPDATE products SET category_id=?, name=?, description=?, price=?, original_price=?, stock=?, image=?, featured=? WHERE id=?");
        $stmt->execute([$categoryId ?: null, $name, $description, $price, $origPrice, $stock, $imagePath, $featured, $id]);
...
<head>
    ...
    <title>Edit Product — BlossomMart Admin</title>
...
<main>
    <div style="max-width: 740px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 28px;">
            ...
            <h1 style="font-size: 24px;">Edit Product #<?= $id ?></h1>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data">
            <div style="background: white; border-radius: 20px; border: 1px solid var(--pink-100); padding: 28px; margin-bottom: 20px;">
                ...
                <div class="field">
                    ...
                    <textarea name="description" class="form-input" rows="4" style="resize: vertical;"><?= htmlspecialchars($product['description']) ?></textarea>
                </div>
            </div>
            ...
            <div style="background: white; border-radius: 20px; border: 1px solid var(--pink-100); padding: 28px; margin-bottom: 20px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div class="field">
                        ...
                        <input type="number" name="price" class="form-input" value="<?= $product['price'] ?>" min="0" step="500" required>
                    </div>
                    <div class="field">
                        <label>Original Price (Rp)</label>
                        <input type="number" name="original_price" class="form-input" value="<?= $product['original_price'] ?>" min="0" step="500">
                    </div>
                </div>
            </div>
            ...
            <div style="background: white; border-radius: 20px; border: 1px solid var(--pink-100); padding: 28px; margin-bottom: 20px;">
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <img src="<?= htmlspecialchars($product['image']) ?>" style="max-height: 140px; border-radius: 12px; border: 1px solid var(--pink-100);">
                </div>
                <?php endif; ?>

                <div class="field">
                    <label>Upload New Image (optional)</label>
                    <div style="border: 2px dashed var(--pink-200); border-radius: 14px; padding: 20px; text-align: center; position: relative; cursor: pointer; transition: all 0.2s;"
                        ...
                        <div id="previewContainer" style="display: none; margin-bottom: 10px;">
                            <img id="imgPreview" style="max-height: 120px; border-radius: 10px; margin: 0 auto;">
                        </div>
                        <i class="ti ti-photo-up" style="font-size: 28px; color: #c2185b; display: block; margin-bottom: 6px;"></i>
                        <p style="font-size: 13px; color: #9e7a90;">Click to replace image</p>
                ...
                <div class="field">
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                        <input type="checkbox" name="featured" <?= $product['featured'] ? 'checked' : '' ?> style="accent-color: #e91e8c; width: 18px; height: 18px;">
